...
def create_default_profile():
    return {
        ...
        "applicable_location": "",
        "applicable_entity": "",
        "use_situational_context": False,
        "use_entity_context": False,
        "use_past_precedents": False
    }
        ...
        if os.path.isdir(profile_path):
            ...
            if os.path.exists(profile_data_path):
                try:
                    ...
                    required_keys = ["pdfs", "priorities", "decision_mode", "custom_text", "last_question", "processed_texts", "applicable_location", "applicable_entity", "use_situational_context", "use_entity_context", "use_past_precedents"]
...
def save_profile(profile_name):
    ...
    profile_data = {
        "priorities": profiles[profile_name]["priorities"],
        ...
        "use_situational_context": profiles[profile_name]["use_situational_context"],
        "use_entity_context": profiles[profile_name]["use_entity_context"],
        "use_past_precedents": profiles[profile_name]["use_past_precedents"]
    }
...
def switch_profile(profile_name):
    ...
    return (
        gr.update(value=profiles[profile_name].get("priorities", ["General"])),
        ...
        gr.update(value=profiles[profile_name].get("use_situational_context", False)),
        gr.update(value=profiles[profile_name].get("use_entity_context", False)),
        gr.update(value=profiles[profile_name].get("use_past_precedents", False)),
...
priority_explanations = {
    "Super Super": "Combine all priorities and provide a concise summary",
    "Super": "Combine selected priorities and provide a concise summary",
...
color_schemes = {
    "Super Super": "#FFA500",  # Orange color for Super Super
    "Super": "#FFD700",  # Gold color for Super
    "General": "#f0f0f0",
...
def ollama_chat(question, context, priority, decision_mode, custom_text, applicable_location, applicable_entity, use_situational_context, use_entity_context, use_past_precedents):
    ...
    mode_instruction = ""
    if decision_mode == "Make Decision":
        mode_instruction = "Help make a decision using the information provided."
    elif decision_mode == "Evaluate Decision":
        mode_instruction = "Evaluate the given decision/idea against the information provided, highlighting any bad ideas and supporting good ideas, and whether ideas support / violate any rules."
    elif decision_mode == "Double Check Decision":
        mode_instruction = "Double check there is nothing wrong in this decision/idea conflicting with the information provided. Don't make a conclusion, just point out flaws."

    situational_context = f"HEAVY WEIGHT ON THIS: Use information from any relevant legislation, laws, regulations or policies or information relevant to location {applicable_location} or territory {applicable_location} is within and therefore also has laws, ideas, policies, regulations applying to (e.g. country, and then global law if applicable)." if use_situational_context else ""
    entity_context = f"Use information from any relevant legislation, laws, regulations or policies or information relevant to the entity {applicable_entity}." if use_entity_context else ""
    past_precedents = "Consider any applicable information or decisions made from past relevant precedent / decisions made in similar situations, including their logic and how that logic might apply here." if use_past_precedents else ""

    formatted_prompt = f"""Your purpose is to be an AI that helps make decisions using data you're provided with and data you can reasonably infer from own knowledge. {priority_instruction}
...
def generate_super_output(results, priorities):
    ...
    try:
        ...
        return f"Error generating super output: {str(e)}"

def ask_question(question, priorities, decision_mode, custom_text, applicable_location, applicable_entity, use_situational_context, use_entity_context, use_past_precedents):
    ...
    try:
        ...
        # Generate outputs for all priorities except Super and Super Super
        for i, priority in enumerate(priority_list[2:], 2):  # Skip "Super Super" and "Super" in this loop
            if priority in priorities or "Super Super" in priorities:
                for partial_result in ollama_chat(question, combined_context, priority, decision_mode, custom_text, applicable_location, applicable_entity, use_situational_context, use_entity_context, use_past_precedents):
        ...
        # Generate Super output if selected
        if "Super" in priorities or "Super Super" in priorities:
            selected_priorities = priorities if "Super" in priorities else priority_list[2:]
            super_results = [r for i, r in enumerate(results[2:]) if priority_list[i+2] in selected_priorities]
            results[1] = generate_super_output(super_results, selected_priorities)
        else:
            results[1] = "Not selected"
        
        # Generate Super Super output if selected
        if "Super Super" in priorities:
            results[0] = generate_super_output(results[2:], priority_list[2:])
        else:
        ...
        history.append({
            ...
            "use_situational_context": use_situational_context,
            "use_entity_context": use_entity_context,
            "use_past_precedents": use_past_precedents,
            "results": results[0],  # Only save Super Super interpretation
...
def display_history():
    ...
    for i, entry in enumerate(reversed(history), 1):
        ...
        <div style="border: 1px solid #ddd; padding: 10px; margin-bottom: 10px; background-color: #f9f9f9;">
            ...
            <p><strong>Priorities:</strong> {', '.join(entry['priorities'])}</p>
            ...
            <p><strong>Use Situational Context:</strong> {entry['use_situational_context']}</p>
            <p><strong>Use Entity Context:</strong> {entry['use_entity_context']}</p>
            <p><strong>Use Precedents for decision-making:</strong> {entry['use_past_precedents']}</p>
...
def save_priorities(priorities):
    profiles[current_profile]["priorities"] = priorities
    save_profile(current_profile)
    return gr.update()
...
def save_use_situational_context(use_situational_context):
    profiles[current_profile]["use_situational_context"] = use_situational_context
    save_profile(current_profile)
    return gr.update()
...
def save_use_past_precedents(use_past_precedents):
    profiles[current_profile]["use_past_precedents"] = use_past_precedents
    save_profile(current_profile)
    return gr.update()
...
""") as demo:
    gr.Markdown("# HiveMind- Your specialised decision-making AI")
    gr.Markdown("Transforming vast amounts of data into decisions.")
                ...
                with gr.TabItem("Main"):
                    with gr.Row():
                        with gr.Column(scale=1):
                            ...
                            priorities = gr.CheckboxGroup(
                                list(priority_explanations.keys()),
                                label="Priorities (Select multiple)",
                                value=["General"]
                            )
                            decision_mode = gr.Radio(
                                ["Make Decision", "Evaluate Decision", "Double Check Decision"],
                                label="Decision Mode",
                                value="Make Decision"
                            )
                            use_past_precedents = gr.Checkbox(label="Consider Past Precedents")
                            with gr.Row():
                                use_situational_context = gr.Checkbox(label="Enable Situational Context")
                            ...
                            custom_text = gr.Textbox(label="Custom Prompt Injection", placeholder="Enter any additional instructions for the AI...")
    ...
    def update_visibility(priorities):
        return [gr.update(visible=priority in priorities or "Super Super" in priorities) for priority in priority_explanations.keys()]

    priorities.change(fn=update_visibility, inputs=[priorities], outputs=output_texts)

    process_button.click(fn=process_pdfs, inputs=[pdf_files], outputs=[status_output])

    def check_processed_pdfs(question, priorities, decision_mode, custom_text, applicable_location, applicable_entity, use_situational_context, use_entity_context, use_past_precedents):
        if not profiles[current_profile]["processed_texts"]:
    ...
    submit_button.click(
        ...
        inputs=[question_input, priorities, decision_mode, custom_text, applicable_location, applicable_entity, use_situational_context, use_entity_context, use_past_precedents],
    ...
    profile_select.change(
        fn=switch_profile,
        inputs=[profile_select],
        outputs=[priorities, decision_mode, custom_text, question_input, applicable_location, applicable_entity, use_situational_context, use_entity_context, use_past_precedents, status_output, pdf_files] + output_texts
    )
    ...
    priorities.change(fn=save_priorities, inputs=[priorities], outputs=[])
    ...
    use_situational_context.change(fn=save_use_situational_context, inputs=[use_situational_context], outputs=[])
    use_entity_context.change(fn=save_use_entity_context, inputs=[use_entity_context], outputs=[])
    use_past_precedents.change(fn=save_use_past_precedents, inputs=[use_past_precedents], outputs=[])
